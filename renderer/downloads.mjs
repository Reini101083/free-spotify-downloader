...
import { translatePage } from './i18n.mjs'
...
export function initializeDownloads() {
  ...
  const update = () => {
    ...
    help.textContent = system.value === 'mac' ? 'Apple-Menü → Über diesen Mac: „Chip“ bedeutet Apple Silicon; „Prozessor: Intel“ bedeutet Intel.' : system.value === 'windows' ? 'Installer: normale Installation. Portable: direkt starten, ohne Installation. Für Intel / AMD 64-Bit.' : 'AppImage: für viele Linux-Distributionen. DEB: für Debian und Ubuntu. Beide für x86_64.'
    button.setAttribute('aria-disabled', String(!item))
    if (item) {
      button.href = packageUrl(item, version.value)
      button.querySelector('span').textContent = 'App herunterladen'
      filename.textContent = packageFile(item, version.value)
    } else {
      button.removeAttribute('href')
      button.querySelector('span').textContent = 'Bitte Mac-Chip wählen'
      filename.textContent = 'Wähle Apple Silicon oder Intel.'
    }
    translatePage()
  }
  const selectSystem = () => {
    packages.replaceChildren()
    if (system.value === 'mac') packages.append(new Option('Bitte Mac-Chip wählen', ''))
    for (const item of PACKAGES.filter(item => item.os === system.value)) packages.append(new Option(item.label, item.id))
    update()
  }
