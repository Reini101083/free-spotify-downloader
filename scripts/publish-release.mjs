...
const notes = `# Free Spotify Downloader ${tag}\n\nBy **Jedi Meister** · Original app code under **MIT**\n\n## Choose your download\n\n${downloadTable(version)}\n\n${installation}\n\n## What's new in 0.2.0\n\n- Responsive interface with light/dark/system appearance, clearer progress, accessible settings and automatic Spotify preview on Start.\n- Default destination: **Downloads/Free Spotify Downloader**. Finished files stay directly in that folder; internal staging stays in app data.\n- **Saved songs** checks real audio files. Zero successful files now means Failed.\n- **Not downloaded** retains individual songs and reasons, with individual and bulk retry. Ordinary song failures continue to the next song.\n- Preserved underlying provider errors, explicit bundled Deno configuration, and a manual YouTube confirmation window that resumes blocked jobs after consent. Rate limits are shown separately.\n- 77 complete offline interface catalogs; German reviewed, other non-English catalogs machine translated. The requested 160-language target is not yet reached.\n- English project documentation and release notes. Node.js 24 build workflows.\n\n## Start\n\n1. Install and open the desktop app.\n2. Copy a Spotify link, switch to the app, or right-click → Paste.\n3. Select Start downloads. Completed files appear under Saved songs; unavailable songs remain retryable.\n\nThe website is a preview; audio downloads run in the desktop app. All four native builds run an offline MP3 encoding, promotion and library-verification test using the packaged engine and bundled tools. These checks do not prove live YouTube availability or full native GUI behavior on every device. Provider restrictions can still prevent downloads.\n\n[All versions](${REPOSITORY}/releases) · [Guide](${REPOSITORY}#readme)\n\nSource: ${process.env.GITHUB_SHA}\n`
...
// Translate only the descriptions of known historical releases; never replace their assets.
for (const oldVersion of ['0.1.1', '0.1.2']) {
  const changes = oldVersion === '0.1.2' ? 'Added native right-click editing, foreground Spotify clipboard import, metadata retry/error reporting and direct platform download links.' : 'Introduced versioned native installers for Windows, macOS and Linux, resumable queues, manual YouTube confirmation and an operating-system download selector.'
  const historicalNotes = `# Free Spotify Downloader v${oldVersion}\n\nBy **Jedi Meister** · Original app code under **MIT**\n\n## Choose your download\n\n${downloadTable(oldVersion)}\n\n${installation}\n\n## Changes\n\n${changes}\n\nThis is a historical release. Version 0.2.0 improves failed-song reporting, saved-file verification, internal staging and the interface. Prefer the [latest release](${REPOSITORY}/releases/latest).\n\nThe website is a preview; audio downloads require the desktop app. Offline checks do not guarantee live provider availability.\n`
  const notesFile = `release-notes-${oldVersion}.md`
  await writeFile(notesFile, historicalNotes)
  execFileSync('gh', ['release', 'edit', `v${oldVersion}`, '--notes-file', notesFile], { stdio:'inherit' })
}
